...
Page({
  data:{
    ...
    weight:[{val:"40kg以下"},{val:"40~60kg"},{val:"60~80kg"},{val:"80kg以上"},],
  ...
  formsubmit:function(e){
    ...
    //准许提交
      ...
      db.collection('questionaire_data').where({_openid : openid}).get({   
        //获取成功，说明用户此前提交过问卷，仅修改提交过的问卷即可 
        success(res){
          db.collection('questionaire_data').where({_openid : openid}).update({ 
            data:{
              gender:e.detail.value.gender,
              age:e.detail.value.age,
              kg:e.detail.value.kg,
